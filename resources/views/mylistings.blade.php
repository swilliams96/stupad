            ...
            @if (count($listings_inactive) > 0)
                ...
                @foreach ($listings_inactive as $listing)
                        <div class="listing listing-inactive">
                            ...
                            <div class="listing-details col-lg-9">
                                <h3><a href="/listings/{{ $listing->id }}/{{ snake_case($listing->title, '-') }}" class="draggable">{{ $listing->title }}</a> <span class="badge inactive" data-toggle="tooltip" data-placement="bottom" title="Listing will not appear in search results unless activated.">Inactive</span></h3>
